        ...
        for key, item in json_data["data"].items():
            ...
            if type(item) is list:
                ...
                for i in item:
                    if type(i) is dict:
                        try:
                            check_required(schema["properties"][key]["items"], i, key, writer)
                            for k, ii, in i.items():
                                check_type(k, ii, schema["properties"][key]["items"], writer)
                        except KeyError:
                            pass
...
def check_required(schema, json_data, obj, writer):
    for item in schema["required"]:
        ...
        if item not in json_data.keys():
            writer.write(f"Не хватает зависимости {item}, пожалуйста добавте {item} в {obj}.")
...
def check_type(key, json_item, schema, writer):
    try:
        ...
        if not check_result:
            writer.write(f"Параметр {key} типа {item_type}, хотя ожидалось {right_type}.")
    except KeyError:
